...
export default function Info({ info }) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:gap-0 gap-10 lg:flex-row items-center lg:px-5 justify-center w-screen">
      <div className="bg-secondary w-screen lg:w-[67%] h-[260px] lg:h-[720px]">
        ...
        />
        <div>
          <Link href={`/anime/${data.id}`} className="hover:underline">
            {title}
          </Link>
          <div></div>
        </div>
      </div>
      <div className="flex flex-col w-screen lg:w-[33%] lg:h-[720px]">
        <h1 className="text-xl font-karla pl-4 pb-5 font-semibold">Episodes</h1>
        <div className="grid gap-5 px-5 lg:h-[720px] h-[595px] py-2 overflow-y-scroll scrollbar-thin scrollbar-thumb-[#313131] scrollbar-thumb-rounded-full">
